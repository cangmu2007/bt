...
int HMACSHA1
	...
	unsigned int lk, // key length
	unsigned char* data, /* data */
	unsigned int ld, // data length
	unsigned char* out, /* output buffer, at least "t" bytes */
	unsigned int *pOutLen // out data len
	)
{
	unsigned int t = 20;
	...
	unsigned char key_large[SHA_DIGESTSIZE];
	...
	if (lk > SHA_BLOCKSIZE)
	{
		SHA1_CTX tctx;
		SHA1Init(&tctx);
		SHA1Update(&tctx, key, lk);
		SHA1Final(key_large, &tctx);
	...
	for (i = 0; i < lk; ++i) buf[i] = key[i] ^ 0x36;
	for (i = lk; i < SHA_BLOCKSIZE; ++i) buf[i] = 0x36;
	SHA1Update(&ictx, buf, SHA_BLOCKSIZE);
	SHA1Update(&ictx, data, ld);
	SHA1Final(isha, &ictx);

	/**** Outter Digest ****/
	SHA1Init(&octx);
	/* Pad the key for outter digest */
	for (i = 0; i < lk; ++i) buf[i] = key[i] ^ 0x5C;
	for (i = lk; i < SHA_BLOCKSIZE; ++i) buf[i] = 0x5C;
	SHA1Update(&octx, buf, SHA_BLOCKSIZE);
	SHA1Update(&octx, isha, SHA_DIGESTSIZE);
	SHA1Final(osha, &octx);

	/* truncate1 and print the results */
	t = t > SHA_DIGESTSIZE ? SHA_DIGESTSIZE : t;
	if(!pOutLen || (pOutLen && *pOutLen < t))
	{
		return -1;
	}
	*pOutLen = t;
