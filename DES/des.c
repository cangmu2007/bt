...
void unscrunch(uint8_t *, uint32_t *);
void scrunch(uint32_t *, uint8_t *);
void deskey(uint32_t *, uint8_t *, int);
static void cookey(uint32_t *, uint32_t *, int);
void desfunc(uint32_t *, uint32_t *);

/*void memset(void* dest_a, int value, int len);
void memset(void* dest_a, int value, int len)
{
    uint8_t* dest = (uint8_t *)dest_a;
    if (len == 0) return;
    do {
...
void DES_CBCInit(DES_CBC_CTX *context, uint8_t *key, uint8_t *iv, int encrypt)
{
...
int DES_CBCUpdate(DES_CBC_CTX *context, uint8_t *output, uint8_t *input, uint32_t len)
{
...
int DESX_CBCUpdate (DESX_CBC_CTX *context, uint8_t *output, uint8_t *input, uint32_t len)
{
    uint32_t inputBlock[2], work[2];
    uint32_t i;
...
// iv ：8字节某某
// Initialize context.  Caller must zeroize the context when finished
void DES3_CBCInit(DES3_CBC_CTX *context, uint8_t *key, uint8_t *iv, int encrypt)
{
...
int DES3_CBCUpdate(DES3_CBC_CTX *context, uint8_t *output, uint8_t *input, uint32_t len)
{
    uint32_t inputBlock[2], work[2];
    uint32_t i;
...
int DES3_ECBUpdate(DES3_CBC_CTX *context, uint8_t *output, uint8_t *input, uint32_t len)
{
    uint32_t inputBlock[2], work[2];
    uint32_t i;
...
void scrunch(uint32_t *into, uint8_t *outof)
{
    *into    = (*outof++ & 0xffL) << 24;
    *into   |= (*outof++ & 0xffL) << 16;
    *into   |= (*outof++ & 0xffL) << 8;
...
void unscrunch(uint8_t *into, uint32_t *outof)
{
...
// Compute DES Subkeys
void deskey(uint32_t subkeys[32], uint8_t key[8], int encrypt)
{
    uint32_t kn[32];
    int i, j, l, m, n;
...
void desfunc(uint32_t *block, uint32_t *ks)
{
